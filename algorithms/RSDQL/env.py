...
class Env():
    ...
    def CalcuVar(self):
        ...
        for i in range(NodeNumber):
            ...
            NodeCPU.append(U)
            NodeMemory.append(M)
            if NodeCPU[i] > float(self.nodeStates[i][1]) or NodeMemory[i] > float(self.nodeStates[i][2]):
                Var = -10  
    ...
    # update state
        ...
        if self.action[0] >= 0 and self.action[1] >= 0:
            if  self.container_state_queue[ self.action[1] * 3 ] == -1:
