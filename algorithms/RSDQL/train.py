...
al_name = 'RSDQL'
file_path = f'E:/research/experiment/experiment_data/baseline/RSDQL/service&placement_{al_name}_{timestamp}.csv'
...
def train(nodeStates):
    ...
    while episode < max_episode: 
        # train part
        for i in tqdm(range(0, 50), desc=f"Training Episode {episode+1}/{max_episode}", ncols=100):  # tqdm进度条
    ...
    return sorted_data, ResultScore

# def get_result(NodeStates, ServiceGraph, ServiceBaseTime, ServiceResource, ServiceContainernum, ContainerRelationship):
def get_result(NodeStates):
    # 训练并获取最终结果
    sorted_data, resultScore = train(NodeStates)
    result_df = pd.read_csv('../../placement/service.csv', header=None)
    ...
    print(sorted_data, resultScore)

NodeStates = remove_first_row('./dataSet/node.csv')
get_result(NodeStates)