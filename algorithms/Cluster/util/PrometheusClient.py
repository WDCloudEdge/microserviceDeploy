...
class PrometheusClient:
    ...
    def collect_request_times(self):
        qps_dic = {}
        prom_url = self.prom_range_url
        request_sql = (
        'sum(rate(istio_requests_total{namespace="%s", reporter="source"}[1m])) by (destination_workload, source_workload)' % self.namespace
    )
        request_times = self.execute_prom(prom_url, request_sql)
        call = []

        if len(request_times) > 0:
            for res in request_times:
                name = res['metric']['source_workload'] + '_' + res['metric']['destination_workload']
                # print(name)
                # times = float(res.get('value')[1])
    ...
    def get_resource_metric_range(self):
        ...
        return metric_df
    def collect_communication_bytes(self):
        request_df = pd.DataFrame()
        response_df = pd.DataFrame()
        request_sql = (
        'sum(rate(istio_request_bytes_sum{namespace="%s", reporter="source", grpc_response_status="0"}[1m])) by (destination_workload, source_workload)' % self.namespace
    )
        response_sql = (
    'sum(rate(istio_response_bytes_sum{namespace="%s", reporter="destination", grpc_response_status="0"}[1m])) by (destination_workload, source_workload)' % self.namespace
)
        request = self.execute_prom(self.prom_range_url, request_sql)
        response = self.execute_prom(self.prom_range_url, response_sql)
        ...
        [handle(result, request_df) for result in request]
        avg_request = request_df.drop(columns='timestamp').mean()
        avg_request_dict = avg_request.to_dict()

        [handle(result, response_df) for result in response]
        avg_response = response_df.drop(columns='timestamp').mean()
        avg_response_dict = avg_response.to_dict()
        result_dict = {key: avg_request_dict.get(key, 0) + avg_response_dict.get(key, 0) for key in avg_request_dict.keys() | avg_response_dict.keys()}
        return result_dict
    ...
    def get_svc_qps_range(self):
        ...
        avg_qps = qps_df.drop(columns='timestamp').mean()

        # 将平均 QPS 转换为字典
        avg_qps_dict = avg_qps.to_dict()
